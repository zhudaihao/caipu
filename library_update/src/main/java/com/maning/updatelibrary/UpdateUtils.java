...
public class UpdateUtils {
    ...
    public LinearLayout rl_layout;//最外层布局
     ...
     * 布局
     */
    public void shownUpDialog(final Activity context, final String appPath, boolean isUpdate) {
        dialog = new Dialog(context, R.style.CursorDialogNotFloatTheme);
        dialog.setContentView(R.layout.dialog_update_app);

        //获取控件
        rl_layout = (LinearLayout) dialog.findViewById(R.id.rl_layout);
        ...
        if (isUpdate) {
            rb_cancel.setVisibility(View.GONE);
        } else {
            rb_cancel.setVisibility(View.VISIBLE);
        }
        ...
        rb_install.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (Build.VERSION.SDK_INT >= 26) {
                    //通过浏览器 下载安装
                    InstallUtils.installAPKWithBrower(context, appPath);

                    //先获取是否有安装未知来源应用的权限（开发版本为27才下面方法）
...
//                    }
    ...
    private static OnClickCancelListener onClickCancelListener;

    public static void setOnClickCancelListener(OnClickCancelListener onClickCancelListener) {
        UpdateUtils.onClickCancelListener = onClickCancelListener;
    }
